...
export function LeadDetail({ lead, orgId, open, onClose }: Props) {
  ...
  // Safe parse of raw_data — guard against null lead
  const parsedRawData = useMemo(() => {
    ...
    if (typeof rawData === "string") {
      try {
        ...
        return { raw_content: rawData } as Record<string, unknown>;
      }
    }
  ...
  // Don't render anything if no lead or dialog not open
  ...
  const currentStatus = (form.status || lead.status || "new") as LeadStatus;
  const stageHistory = (lead.stage_history || {}) as Record<string, string>;
              ...
              <div className="space-y-2.5">
                ...
                {stages.length > 0 ? (
                  <div className="relative pl-4 border-l border-border/80 ml-1.5 space-y-3">
                    {stages.map((stage) => {
                      const entryDateStr = stageHistory[stage.id];
                      ...
                      return (
                        <div key={stage.id} className="relative flex items-center justify-between text-xs py-0.5">
                          ...
                          <span className={`font-medium ${isCurrent ? "text-foreground font-semibold" : "text-muted-foreground"}`}>
                            {stage.name}{" "}
                            {isCurrent && (
                              <span className="text-[9px] ml-1.5 px-1.5 py-0.5 rounded bg-primary/15 text-primary font-medium">Atual</span>
                            )}
                          </span>
                          <span className="text-muted-foreground font-mono">
                            {entryDate && !isNaN(entryDate.getTime()) ? entryDate.toLocaleString("pt-BR") : "—"}
                          </span>
                        </div>
                      );
                    })}
